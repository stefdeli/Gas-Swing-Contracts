...
def _build_constraints_elecDA_seq(self):
    ...
    for t in time:
        ...
        lhs =      gb.quicksum(var.WindDA[wf,t] for wf in windfarms) \
                +  gb.quicksum(var.Pgen[gen,t] for gen in generators)\
                + gb.quicksum(var.PgenSC[gen,sc,t] for gen in gfpp for sc in swingcontracts)
        rhs =  + self.edata.sysload[t]
        add_constraint(self,lhs,'==',rhs,name)
    ...
    for t in time:
        for i in gfpp:
            for sc in swingcontracts:
                ...
                name = 'PgenSCmax({0},{2},{1})'.format(i,t,sc)
                self.constraints[name]= expando()
                cc=self.constraints[name]
                cc.lhs = var.PgenSC[i,sc,t]
                cc.rhs = usc[sc] * self.edata.SCP[(sc,i),t] * SCdata.PcMax[sc,i]
                cc.expr = m.addConstr(cc.lhs <= cc.rhs,name=name)
                
                
                name = 'PgenSCmin({0},{2},{1})'.format(i,t,sc)
                self.constraints[name]= expando()
                cc=self.constraints[name]
                
                cc.lhs= - var.PgenSC[i,sc,t]
                cc.rhs= - usc[sc] * self.edata.SCP[(sc,i),t] * SCdata.PcMin[sc,i]
                cc.expr = m.addConstr(cc.lhs <= cc.rhs,name=name)

                     
                name = 'RCupSCmax({0},{2},{1})'.format(i,t,sc)
                self.constraints[name]= expando()
                cc=self.constraints[name]
                cc.lhs= var.RCupSC[i,sc,t]  + var.PgenSC[i,sc,t] - usc[sc] * self.edata.SCP[(sc,i),t] * SCdata.PcMax[sc,i]
                cc.rhs= np.float64(0.0)
                cc.expr = m.addConstr(cc.lhs <= cc.rhs,name=name)
                
                
                name = 'RCdnSCmin({0},{2},{1})'.format(i,t,sc)
                self.constraints[name]= expando()
                cc=self.constraints[name]
                cc.lhs = var.RCdnSC[i,sc,t] -var.PgenSC[i,sc,t] + usc[sc] * self.edata.SCP[(sc,i),t] *  SCdata.PcMin[sc,i]
                cc.rhs = np.float64(0.0)
                cc.expr = m.addConstr(cc.lhs <= cc.rhs,name=name)
...
def _build_constraints_elecRT_seq(self,mtype,dispatchElecDA):
    ...
    for s in scenarios:                
        for t in time:
            ...
            lhs =    gb.quicksum(var.RUp[g,s,t] - var.RDn[g,s,t] for g in generators)  \
                      + gb.quicksum(var.RUpSC[g,s,sc,t] - var.RDnSC[g,s,sc,t] for g in gfpp for sc in swingcontracts)   \
                      - gb.quicksum(WindDA[w,t] for w in windfarms) \
                      - gb.quicksum(var.Wspill[w,s,t] for w in windfarms)  \
                      + var.Lshed[s,t]
            rhs =  - sum(wscen[w][scenwind[s]][t]*wcap[w] for w in windfarms)
        ...
        for t in time:
            for i in gfpp:
                for sc in swingcontracts:
                    ...
                    name = 'RegUpSC_max_RT({0},{1},{3},{2})'.format(i,s,t,sc)
                    self.constraints[name]= expando()
                    cc=self.constraints[name]
                    if mtype=='Stoch': # Then RCup is variable and needs to be on LHS
                        cc.lhs=var.RUpSC[i,s,sc,t]-RCupSC[i,sc, t]
                        cc.rhs=np.float64(0.0)
                    elif mtype == 'RealTime':
                        cc.lhs=var.RUpSC[i,s,sc,t]
                        cc.rhs=RCupSC[i,(t,sc)]
                        
                    cc.expr = m.addConstr(cc.lhs <= cc.rhs,name=name)
                    
                    name = 'RegDnSC_max_RT({0},{1},{3},{2})'.format(i,s,t,sc)
                    self.constraints[name]= expando()
                    cc=self.constraints[name]
                    if mtype=='Stoch': # Then RCup is variable and needs to be on LHS
                        cc.lhs=var.RDnSC[i,s,sc,t]-RCdnSC[i,sc, t]
                        cc.rhs=np.float64(0.0)
                    elif mtype == 'RealTime':
                        cc.lhs=var.RDnSC[i,s,sc,t]
                        cc.rhs=RCdnSC[i,(t,sc)]
                    cc.expr = m.addConstr(cc.lhs <= cc.rhs,name=name)
